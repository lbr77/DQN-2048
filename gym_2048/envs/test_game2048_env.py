...
class TestBoard:
    ...
    def test_move(self):
        ...
        b = game2048_env.Game2048Env()
        # Test shift up
        b.set_board(np.array([[0, 2, 0, 4], [2, 2, 8, 0], [2, 2, 2, 8], [2, 2, 4, 4]]))
        assert b.move(0) == 12
        assert np.array_equal(
            b.get_board(),
            np.array([[4, 4, 8, 4], [2, 4, 2, 8], [0, 0, 4, 4], [0, 0, 0, 0]]),
        )
        # Test shift right
        b.set_board(np.array([[0, 2, 0, 4], [2, 2, 8, 0], [2, 2, 2, 8], [2, 2, 4, 4]]))
        assert b.move(1) == 20
        assert np.array_equal(
            b.get_board(),
            np.array([[0, 0, 2, 4], [0, 0, 4, 8], [0, 2, 4, 8], [0, 0, 4, 8]]),
        )
        # Test shift down
        b.set_board(np.array([[0, 2, 0, 4], [2, 2, 8, 0], [2, 2, 2, 8], [2, 2, 4, 4]]))
        assert b.move(2) == 12
        assert np.array_equal(
            b.get_board(),
            np.array([[0, 0, 0, 0], [0, 0, 8, 4], [2, 4, 2, 8], [4, 4, 4, 4]]),
        )
        # Test shift left
        b.set_board(np.array([[0, 2, 0, 4], [2, 2, 8, 0], [2, 2, 2, 8], [2, 2, 4, 4]]))
        assert b.move(3) == 20
        assert np.array_equal(
            b.get_board(),
            np.array([[2, 4, 0, 0], [4, 8, 0, 0], [4, 2, 8, 0], [4, 8, 0, 0]]),
        )
        ...
        assert np.array_equal(
            b.get_board(),
            np.array([[0, 4, 0, 0], [2, 8, 0, 0], [4, 2, 0, 0], [8, 8, 8, 0]]),
        )

    def test_highest(self):
        b = game2048_env.Game2048Env()
        b.set_board(
            np.array([[0, 2, 0, 4], [2, 2, 8, 0], [2, 2, 2048, 8], [2, 2, 4, 4]])
        )
        assert b.highest() == 2048

    def test_isend(self):
        b = game2048_env.Game2048Env()
        b.set_board(np.array([[2, 2, 2, 2], [2, 2, 2, 2], [2, 2, 2, 2], [2, 2, 2, 2]]))
        assert b.isend() == False
        b.set_board(
            np.array([[2, 4, 8, 16], [4, 8, 16, 2], [8, 16, 2, 4], [16, 2, 4, 8]])
        )
        assert b.isend() == True
